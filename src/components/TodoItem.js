...
class TodoItem extends React.Component {
  ...
  updateClick(e) {
  	...
  	this.props.onUpdateClick(this.state.newItem, this.state.newDate, this.state.newNote)
  	this.setState({open: false})
...
// this.setState({newItem: item, newNote: note})
  render()	{
  	let {onItemClick, onDeleteClick, completed, deleted, item, formattedDate, note} = this.props;
	...
	return (<div>
			<Grid xs={12}>
		        <Badge badgeContent={formattedDate} 
		        ...
		        style={{display: deleted ? 'none' : 'inline-block'}}>
					...
					>
					        ...
					        style={{display: !note ? 'none' : 'block', textAlign: 'left', backgroundColor: '#E5E5E5'}}>
					        ...
					        <div style={{float: 'right'}}>
					          <Button color={!completed ? 'primary' : 'default'} onClick={onItemClick}>
					          	{!completed ? 'complete' : 'incomplete'}
						      </Button>
							  <Button disabled={!completed ? false : true} color="primary" onClick={this.handleClickOpen}>
	      						Edit
					          </Button>
